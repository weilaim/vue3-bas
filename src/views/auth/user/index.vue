    ...
    <n-space class="pb-12px" justify="space-between">
      <n-space>
        <n-button type="primary" @click="handleAddTable">
          <icon-ic-round-plus class="mr-4px text-20px"></icon-ic-round-plus>
          新增
        </n-button>
        <n-button type="error">
          <icon-ic-round-delete class="mr-4px text-20px"></icon-ic-round-delete>
          删除
        </n-button>
        <n-button type="success">
          <icon-uil:export class="mr-4px text-20px"></icon-uil:export>
          导出Excel
        </n-button>
      </n-space>
      <n-space align="center" :size="18">
        <n-button size="small" type="primary">
          <icon-mdi-refresh class="mr-4px text-16px" :class="{ 'animate-spin': loading }"></icon-mdi-refresh>
          刷新表格
        </n-button>
        <column-setting v-model:columns="columns"></column-setting>
      </n-space>
    </n-space>
    <n-data-table :columns="columns" :data="tableData" :loading="loading" :pagination="pagination" />
    <table-action-modal v-model:visible="visible" :type="modalType" :edit-data="editData"></table-action-modal>
  </n-card>
</template>

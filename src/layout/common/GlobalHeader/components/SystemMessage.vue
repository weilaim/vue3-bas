  ...
  <n-popover class="!p-0" trigger="click" placement="bottom">
    ...
    >
      <n-tab-pane v-for="(item, index) in tabData" :key="item.key" :name="index">
        <template #tab>
          <div class="flex-x-center items-center m-5" :class="[isMobile ? 'w-92px' : 'w-120px']">
        ...
        <loading-empty-wrapper
          ...
          placeholder-class="bg-$n-color transition-background-color dutation-300 ease-in-out"
        >
    ...
    <div v-if="showAction" class="flex border-t border-$n-divider-color cursor-pointer">
      ...
      <div class="flex-1 text-center py-10px border-1 border-$n-divider-color" @click="handleAllRead">全部已读</div>
      <div class="flex-1 text-center py-10px border-1 border-$n-divider-color" @click="handleLoadMore">查看更多</div>
    </div>
  </n-popover>
</template>

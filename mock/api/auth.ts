import { REFRESH_TOKEN_CODE } from './../../src/config/service';
import { userModel } from '../model/auth';
import { MockMethod } from "vite-plugin-mock"

/** 参数错误的状态码 */
const ERROR_PARAM_CODE = 1000
...
const apis:MockMethod[] = [
  ...
  // 用于用户信息(请求头携带token,根据token获取用户信息)
  { 
    ...
    response:(options:Service.MockOption):Service.MockServiceResult<ApiAuth.UserInfo | null> => {
      ...
      const isInUser = userModel.some(item => {
        ...
        if(flag){
          const {userId:itemUserId,userName,userRole,avatar} = item
          Object.assign(userInfo,{userId:itemUserId,userName,userRole,avatar})
        }
        return flag
      });
      if(isInUser){
